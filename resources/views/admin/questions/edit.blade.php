            ...
            <div>
                <div class="flex flex-wrap items-center justify-between gap-4">
                    <div>
                        ...
                        <h1 class="mt-2 text-2xl font-bold text-gray-900">{{ __('admin.questions.edit_title') }}</h1>
                        <p class="mt-2 text-sm text-gray-500">{{ __('admin.questions.edit_subtitle') }}</p>
                    </div>
                </div>
                ...
                <form method="POST" action="{{ route('admin.questions.update', $question) }}" class="mt-8">
                    @csrf
                    @method('PUT')
                    @include('admin.questions._form', ['question' => $question, 'categories' => $categories])
                </form>
            </div>
        </div>
    </main>
